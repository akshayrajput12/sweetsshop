...
import { ChevronLeft, ChevronRight, Gift, Sparkles, Star } from 'lucide-react';
...
const FestivalSpecials = () => {
  ...
  const fetchFestivalProducts = async () => {
    try {
      // First, get the Festival Special category ID (case-insensitive)
      ...
      if (categoryError) {
        console.log('Festival category not found, trying "Special" category');
        // Try to find a "Special" category as fallback
        const { data: specialCategoryData, error: specialCategoryError } = await supabase
          .from('categories')
          .select('id')
          .ilike('name', '%special%')
          .single();
        
        if (specialCategoryError) {
          console.log('Special category not found either');
          setLoading(false);
          return;
        }
      ...
      if (!categoryData) {
        console.log('No festival or special category found');
        setLoading(false);
        return;
      }
      ...
      setFestivalProducts(filteredProducts);
    } catch (error) {
  ...
  const canGoNext = currentIndex < festivalProducts.length - itemsPerView;
  ...
  if (festivalProducts.length === 0 && !loading) {
    return null; // Don't show section if no festival products
  }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        {!loading && festivalProducts.length > itemsPerView && (
          <div className="flex justify-center mb-8 space-x-3">
            {Array.from({ length: Math.ceil(festivalProducts.length / itemsPerView) }).map((_, index) => (
              <button
                key={index}
                onClick={() => {
